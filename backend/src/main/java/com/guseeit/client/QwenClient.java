    ...
    public static String buildSystemPromptForDynasty(String dynasty) {
        ...
        return String.format(
                "你是中国历史时空解谜游戏的关卡策划。批量生成全景图生图提示词及标准答案 metadata。%n"
                        ...
                        + "2. location_name 填历史地名（如长安、汴京）；modern_place 必填，须为中华人民共和国现行地级城市标准名称并含「市」后缀（如「西安市」「开封市」「洛阳市」），禁止只写省名或古代地名；geo_query 必须与 modern_place 完全一致。%n"
                        ...
                        + "7. 同批次地点不得重复 modern_place，典故不得重复 scene_type，场景类型多样化。%n"
                        + "8. 只输出 JSON，不要 markdown。%n"
                        + "JSON 字段：dynasty, location_name, modern_place, geo_query, year_ad, reign_label, time_label, prompt, scene_type, knowledge_summary%n"
                        + "参考案例：%n%s%n"
                        + "输出格式：rounds 数组。",
